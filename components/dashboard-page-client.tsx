...
import { type FunctionType, type Stage, type Priority, getStageLabel } from "@/lib/mock-data"
...
export function DashboardPageClient({ initialOpportunities }: DashboardPageClientProps) {
  ...
  console.log("[v0] DashboardPageClient: Rendering with opportunities:", initialOpportunities.length)

  const filteredOpportunities = useMemo(() => {
    return initialOpportunities.filter((opp) => {
      // Year filter
      if (year !== "all") {
        const oppYear = new Date(opp.created_at || "").getFullYear().toString()
        if (oppYear !== year) return false
      }

      // Function filter
      if (functionFilter !== "all" && opp.function !== functionFilter) return false
      ...
      if (search) {
        ...
        return opp.name.toLowerCase().includes(searchLower) || opp.id.toLowerCase().includes(searchLower)
      }

      return true
    })
  }, [initialOpportunities, year, functionFilter, stageFilter, priorityFilter, search])

  const sortedOpportunities = useMemo(() => {
    if (!sortField || !sortDirection) return filteredOpportunities
    ...
    return [...filteredOpportunities].sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "priority":
          ...
          aValue = priorityWeight[a.priority as keyof typeof priorityWeight] || 0
          bValue = priorityWeight[b.priority as keyof typeof priorityWeight] || 0
          break
        case "owner":
          aValue = a.owner
          bValue = b.owner
              ...
              <TableBody>
                {sortedOpportunities.map((opp) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <PriorityBadge priority={opp.priority as Priority} />
                    </TableCell>
                    <TableCell>
